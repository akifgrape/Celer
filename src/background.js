...
chrome.runtime.onInstalled.addListener((object) => {
  ...
  if (object.reason === "install") {
    chrome.tabs.create({ url: "https://eighred.com/" });
  }
});

// Check when the extension button is clicked
chrome.action.onClicked.addListener((tab) => {
	chrome.tabs.sendMessage(tab.id, {request: "open-Celer"});
});
	...
	if (command === "open-Celer") {
		getCurrentTab().then((response) => {
			if (!response.url.includes("chrome://") && !response.url.includes("chrome.google.com")) {
				chrome.tabs.sendMessage(response.id, {request: "open-Celer"});
			} else {
				chrome.tabs.create({
					url: "./newtab.html"	
				}).then(() => {
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => resetCeler());
chrome.tabs.onCreated.addListener((tab) => resetCeler());
chrome.tabs.onRemoved.addListener((tabId, changeInfo) => resetCeler());
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	switch (message.request) {
		...
		case "close-Celer":
			getCurrentTab().then((response) => {
				chrome.tabs.sendMessage(response.id, {request: "close-Celer"});
			});
			break;
		}
});

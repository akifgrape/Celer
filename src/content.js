...
document.onkeyup = (e) => {
	if (e.key == "Escape" && isOpen) {
		chrome.runtime.sendMessage({request:"close-Celer"})
	}
}
...
$(document).ready(() => {
	...
	$.get(chrome.runtime.getURL('/content.html'), (data) => {
		...
		$("#Celer-extension-toast img").attr("src", chrome.runtime.getURL("assets/check.svg"));
	...
	function renderAction(action, index, keys, img) {
		...
		if (!action.emoji) {
			...
			loadimg.onerror = () => {
				$(".Celer-item[data-index='"+index+"'] img").attr("src", chrome.runtime.getURL("/assets/globe.svg"));
			}
		}
	}
	...
	function populateCeler() {
		...
		actions.forEach((action, index) => {
			...
			if (!action.emoji) {
				...
				var img = "<img src='"+action.favIconUrl+"' alt='favicon' onload='"+onload+"' onerror='this.src=&quot;"+chrome.runtime.getURL("/assets/globe.svg")+"&quot;' class='Celer-icon'>";
	...
	function populateCelerFilter(actions) {
		...
		const renderRow = (index) => {
			...
			var img = "<img src='"+action.favIconUrl+"' alt='favicon' onerror='this.src=&quot;"+chrome.runtime.getURL("/assets/globe.svg")+"&quot;' class='Celer-icon'>";
			if (action.emoji) {
	...
	function handleAction(e) {
		...
		if ($(".Celer-extension input").val().toLowerCase().startsWith("/remove")) {
			...
			chrome.runtime.sendMessage({request:action.action, tab:action, query:$(".Celer-extension input").val()});

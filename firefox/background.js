...
const clearActions = () => {
	getCurrentTab().then((response) => {
		...
		actions = [
			...
			{title:"Help", desc:"Get help with Celer on GitHub", type:"action", action:"url", url:"https://github.com/akifgrape/Celer", emoji:true, emojiChar:"🤔", keycheck:false},
...
browser.runtime.onInstalled.addListener((object) => {
  ...
  if (object.reason === "install") {
    browser.tabs.create({ url: "https://github.com/akifgrape/Celer" });
  }
});

// Check when the extension button is clicked
browser.browserAction.onClicked.addListener((tab) => {
	browser.tabs.sendMessage(tab.id, {request: "open-Celer"});
});

// Listen for the open Celer shortcut
browser.commands.onCommand.addListener((command) => {
	if (command === "open-Celer") {
		getCurrentTab().then((response) => {
			if (!response.url.includes("browser://") && !response.url.includes("browser.google.com")) {
				browser.tabs.sendMessage(response.id, {request: "open-Celer"});
			} else {
				browser.tabs.create({
					url: "./newtab.html" 
				}).then(() => {
...
// Restore the new tab page (workaround to show Celer in new tab page)
...
browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => resetCeler());
browser.tabs.onCreated.addListener((tab) => resetCeler());
browser.tabs.onRemoved.addListener((tabId, changeInfo) => resetCeler());
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
	switch (message.request) {
		...
		case "close-Celer":
			getCurrentTab().then((response) => {
				browser.tabs.sendMessage(response.id, {request: "close-Celer"});
			});
			break;
		}
});
